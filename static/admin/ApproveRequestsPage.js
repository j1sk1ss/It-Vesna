document.addEventListener("DOMContentLoaded", function () {
    ...
    const approveRequests = JSON.parse(localStorage.getItem("approveRequests")) || [];
    const selectedRequest = approveRequests.find(request => request.id === parseInt(requestId));
...
function handleDelete() {
    const urlParams = new URLSearchParams(window.location.search);
    const requestId = urlParams.get("id");
    let approveRequests = JSON.parse(localStorage.getItem("approveRequests")) || [];
    approveRequests = approveRequests.filter(request => request.id !== parseInt(requestId));
    localStorage.setItem("approveRequests", JSON.stringify(approveRequests));
    window.location.href = "/admin-panel.html";
}

function toggleHover(state) {
    document.querySelector(".hovering-buttons2").style.transform = state ? "translateX(15%)" : "translateX(100%)";
}
